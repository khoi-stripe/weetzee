...
export function isFullHouseN(dice: number[]): boolean {
  const vals = Object.values(counts(dice)).sort((a, b) => a - b);
  if (vals.length !== 2) return false;
  return vals[0] >= 2 && vals[1] >= 3;
}

export function isLargeStraightN(dice: number[]): boolean {
...
export function makeClassicCategories(diceCount: number): ScoreCategory[] {
  const is6 = diceCount === 6;
  const maxSum = 6 * diceCount;

  const upper: ScoreCategory[] = [1, 2, 3, 4, 5, 6].map((face) => ({
    id: `upper_${face}`,
    name: ["Ones", "Twos", "Threes", "Fours", "Fives", "Sixes"][face - 1],
  ...
  const lower: ScoreCategory[] = [
    ...
    },
    {
      id: "four_of_a_kind",
      name: "4 of a kind",
      evaluate: (dice: number[]) => (hasNOfAKind(dice, 4) ? sum(dice) : null),
      maxScore: maxSum,
    },
    {
      id: "sm_straight",
      name: "Sm. straight",
      evaluate: (dice: number[]) => (isSmallStraight(dice) ? 30 : null),
      maxScore: 30,
    },
    {
      id: "lg_straight",
      name: "Lg. straight",
      evaluate: is6
        ? (dice: number[]) => (isLargeStraightN(dice) ? 40 : null)
        : (dice: number[]) => (isLargeStraight(dice) ? 40 : null),
      maxScore: 40,
    },
    {
      id: "full_house",
      name: "Full house",
      evaluate: is6
        ? (dice: number[]) => (isFullHouseN(dice) ? 25 : null)
        : (dice: number[]) => (isFullHouse(dice) ? 25 : null),
      maxScore: 25,
    },
    {
      id: "chance",
    ...
    },
    {
      id: "weetzee",
      name: "Weetzee",
      evaluate: (dice: number[]) => (hasNOfAKind(dice, diceCount) ? 50 : null),
      maxScore: 50,
    },
    {
      id: "bonus",
